...
gets pinned SGLang `v0.5.19` from source with its Metal dependencies:
...
default. Use `vocoder.factory.mlx_model_path` when Flow/HiFT weights live in a
different converted artifact. Both paths accept a corresponding
`mlx_model_revision` override.

The native vocoder expects a converted MLX artifact containing `config.json`
and `model.safetensors`; raw `flow.pt`/`hift.pt` checkpoints are not loaded by
this path. The stage device must remain MPS.
