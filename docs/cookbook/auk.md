...
Base AuK can materialize DiT Linear and Conv1d compute parameters in BF16 once at load time with `--auk_engine.factory.precast_dit_compute_weights true`. The option is disabled by default and leaves normalization parameters, explicit FP32 state, Qwen conditioning, and the VAE unchanged.
