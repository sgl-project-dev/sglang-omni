...
def _same_gpu_frontend_replica_config(tmp_path) -> HiggsTtsPipelineConfig:
    ...
    tts_engine_idx = next(
        i for i, stage in enumerate(stages_cfg) if stage.name == "tts_engine"
    )
    stages_cfg[tts_engine_idx] = stages_cfg[tts_engine_idx].model_copy(
        update={"gpu_memory_fraction": 0.82}
    )
...
def test_higgs_frontend_replicas_inject_same_gpu_id(tmp_path) -> None:
    ...
    for replica_id in range(2):
        ...
        factory = import_string(audio_encoder.factory)
        factory_args = apply_typed_stage_kwargs(
            factory,
            audio_encoder.factory_kwargs,
            audio_encoder.typed_kwargs,
