...
async def test_mounted_created_strict_recovery_eos_and_ack():
    async with endpoint() as (http, url, producer, app):
        ...
        async with websockets.connect(url) as ws:
            ...
            assert (
                cancelled["type"] == "sglang.response.cancelled"
            )  # Note (Junnan Li): ACK emitted no reply.
...
@pytest.mark.asyncio
async def test_concurrent_input_overflow_retry_clear_partial_and_timeout():
    producer = Producer()
    ...
    class ASR(Producer):
        async def process(self, unit, epoch):
            for seg in ("s1", "s2"):
                ...
                await self.emit(TranscriptionRevision(text="helo", **fields), epoch)
                await self.emit(
                    TranscriptionRevision(
