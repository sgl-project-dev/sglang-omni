...
async def test_shared_turn_adapter_uses_existing_completion_and_retained_clear():
    ...
    async with endpoint(deployment=deployment) as (_, url, _, app):
        async with websockets.connect(url) as ws:
            ...
            )  # Note (Junnan Li): existing response then transcription
            assert any(e["type"] == "response.output_text.delta" for e in events)


@pytest.mark.asyncio
async def test_coordinator_native_public_unit_commit(tmp_path):
    ...
    async with pipeline(tmp_path) as (coordinator, _, _):
        ...
        async with endpoint(deployment=deployment) as (_, url, _, app):
            async with websockets.connect(url) as ws:
                ...
                await recv(ws)
                assert created["session"]["id"] in coordinator._sessions
        ...
        try:
            async with httpx.AsyncClient() as client:
                for _ in range(200):
                    ...
                    raise AssertionError("router readiness timeout")
