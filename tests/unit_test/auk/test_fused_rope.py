...
def test_rope_matches_eager(batch, seq_len, shared):
    from sglang_omni.models.auk.fused_rope import fused_rope

    torch.manual_seed(1234)
    q = torch.randn(batch, 24, seq_len, 64, device="cuda")
    k = torch.randn_like(q)
    # A longer table also checks x-transformers' suffix selection.
    freqs = torch.randn(1 if shared else batch, seq_len + 3, 64, device="cuda") * 500
    expected = Attention._apply_rope(q, k, (freqs, 1.0))
    actual = fused_rope(q, k, freqs)
...
def test_attention_matches_eager(head_dim, with_rope):
    torch.manual_seed(7)
