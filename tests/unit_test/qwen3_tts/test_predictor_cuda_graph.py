...
def test_startup_capture_follows_the_platform_and_the_resolved_config(
    ...
    """The startup capture needs a graph backend, single-rank TP, and an
    undeclared disable_cuda_graph."""
    ...
    # Every step is eager under TP, so the startup set must not be recorded either.
    tp.tp_size = 2
    assert not _should_capture_predictor_graphs(server_args=enabled, device="cuda:0")


def test_a_graph_signature_is_reachable_off_cuda():
    """The signature gate must admit the accelerator the predictor cache is on,
    or a captured graph is never replayed there."""
...
def test_both_gates_reject_another_card_of_the_same_kind() -> None:
    """A graph's buffers live on one card. Admitting xpu:1 while the cache is on
    xpu:0 would copy the batch across and run it on the wrong card, so the gates
    compare the whole device rather than its kind."""
    talker = _build_talker(torch.device("cpu"))
    talker.prepare_decode_buffers(_uniform_requests(2))
...
def test_a_capture_that_fails_after_the_graph_exists_releases_it(
    ...
    """The graph belongs to the platform's backend, so the cleanup path can only
    reset what capture yielded. Runs anywhere: the device and the backend are
    fakes, which is what the CUDA-marked twin cannot be."""
    resets: list[int] = []
...
def test_a_step_on_a_device_without_a_graph_backend_stays_eager(
    ...
    """No platform records a model-owned graph on CPU, and torch.cpu has no
    is_current_stream_capturing, so the dispatch must decline before it asks."""
    talker = _build_talker(torch.device("cpu"))
...
def test_capture_failure_resets_cuda_graph(monkeypatch: pytest.MonkeyPatch):
    ...
    # The failure has to land inside the recorded region: the platform's backend
    # owns the graph, so nothing exists to reset until capture has yielded one.
    real_forward = Qwen3TTSTalker._code_predictor_forward_incremental
...
def test_resolve_predictor_graph_enabled(monkeypatch: pytest.MonkeyPatch):
    talker = object.__new__(Qwen3TTSTalker)
    talker._predictor_k_cache = torch.zeros(1)
