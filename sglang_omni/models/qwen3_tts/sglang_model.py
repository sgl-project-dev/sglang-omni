...
def predictor_graph_policy_enabled() -> bool:
    """Whether recording the predictor chain is wanted here, before any device
    question. The startup capture and the per-step dispatch both read this, so
    the work one does matches what the other will use."""
    # Note: (Jiaxin Deng) capture under TP would record collectives; the graphed
    # chain is only validated single-rank, so TP stays eager.
...
class _PredictorDecodeGraph:
    ...
    cyclic collector. `graph` is None until the platform's backend records it.
    """
...
class Qwen3TTSTalker(Qwen3TTSPromptBuilderMixin, nn.Module):
    ...
    def _resolve_predictor_graph_enabled(self) -> bool:
        if not predictor_graph_policy_enabled():
            return False
        if (
            current_platform.get_device_graph_backend(self._predictor_k_cache.device)
            is None
        ):
            return False
    ...
    def _predictor_forward_graphed(
        ...
        if layer0_codes.dtype not in (torch.int, torch.long):
            return None
        # The full device, not its type: the graph's buffers live on one card, and
        # copying a same-kind tensor in from another would run the batch there.
