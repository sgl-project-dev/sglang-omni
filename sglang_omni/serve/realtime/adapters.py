...
from sglang_omni.serve.realtime.runtime import ContextLimitError, RuntimeLimits
...
class CoordinatorAdapter:
    ...
    async def _read(self):
        try:
            # Note (Junnan Li): This iterator survives abort. Replacing/closing it closes A's owner.
    ...
    async def cancel(self):
        # Note (Junnan Li): The coordinator completes the unit and preserves its receipt.
        future = self.future
...
class TurnBasedAdapter:
    ...
    async def open(self, session_id, config, emit):
        ...
        from sglang_omni.serve.realtime.session import TurnBasedSession
    ...
    async def update(self, config):
        ...
        from sglang_omni.serve.realtime.session import TurnConfigurationError
    ...
    def _apply_pending_update(self):
        pending = getattr(self, "pending_update", None)
        if pending is None:
            return
        candidate, detector = pending
    ...
    async def process(self, unit, epoch):
        ...
        if unit.eos and not self.engine.audio_buffer.is_empty():
            # Note (Junnan Li): EOS finalizes the retained utterance; explicit commit/create are
            # deliberately not granted by this adapter.
            end_offset = (
        ...
        if unit.eos and self.engine.queue_drainer is not None:
            while (
                self.engine.active_task is not None
                or not self.engine.response_queue.empty()
            ):
    ...
    async def cancel(self):
        self.output_epoch += 1
        # Note (Junnan Li): Preserve the active turn's result and history. Runtime fences its
        # output while subsequent accepted turns start in the new epoch.
