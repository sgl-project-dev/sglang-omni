...
class InteractionAdapter(Protocol):
    async def open(self, session_id: str, config: dict, emit: Callable): ...
...
class SessionRuntime:
    ...
    def _enqueue(self, envelope: Envelope):
        # Note (Junnan Li): repr bounds typed output before JSON/base64 expansion at the boundary.
    ...
    async def emit(self, event: OutputEvent, epoch: int, unit: Unit | None = None):
        ...
        # Note (Junnan Li): A hot update applies to subsequent work, including queued wire output.
        ...
        if isinstance(event, (AudioDelta, AudioFinished)) and "audio" not in modalities:
            return
        if isinstance(event, (TextDelta, TextFinished)) and not modalities:
            return
    ...
    async def update(self, patch, event_id):
        async with self.lock:
            ...
            if self.state == "CREATED":
                ...
                if hasattr(adapter, "set_limits"):
                    adapter.set_limits(self.limits)
                if hasattr(adapter, "set_cancel_handler"):
                ...
                if self.close_task is not None:
                    # Note (Junnan Li): The closing owner waits for this admission and then releases
                    # it. Never publish OPEN or start work after close was requested.
                    return
                self.state = "OPEN"
                self.worker = asyncio.create_task(self._pump())
            elif candidate != self.config:
                updater = getattr(self.adapter, "update", None)
                if updater is None:
                    before = (
    ...
    async def _pump(self):
        ...
        try:
            ...
            self.fail(str(exc), getattr(exc, "code", "internal"))
    ...
    def _finish_responses(self, old, status, reason):
        ...
        for rid, state in self.responses.items():
            ...
            for event in events:
                # Note (Junnan Li): At most three terminal events per bounded response ledger.
                envelope = Envelope(
                    old,
                    event,
                    True,
    ...
    def before_send(self, envelope: Envelope):
        """Reserve lifecycle order before yielding to the single socket sender."""
    ...
    def _terminal(self, event):
        # Note (Junnan Li): Two reserved terminal slots cannot be occupied by media producers.
        terminals = [
            (env, size)
    ...
    def fail(self, message, code="internal", event_id=None):
        if self.close_task is not None:
            return
        # Note (Junnan Li): Failure/close control bypasses the exhausted data queue by discarding it.
    ...
    async def _close(self, reason, event_id=None):
        # Note (Junnan Li): Serialize with admission/cancel, then release the command lock before
        # awaiting adapter teardown (a VAD callback may need to observe CLOSING).
        async with self.lock:
            self.state = "CLOSING"
    ...
    async def _close_state(self, reason, event_id=None):
        ...
        try:
            if cleanup_error is not None:
                ...
                status = (
                    "cancelled"
                    if reason in ("client_closed", "disconnect", "test")
                    else "failed"
                )
