...
class TurnBasedSession:
            ...
            try:
                ...
                if self.capture_turn_context and isinstance(results[0], Exception):
                    await self.emit_output(
                        TurnFailure(
                            "server_error",
                            getattr(results[0], "code", "internal"),
                            str(results[0]),
                        )
                    )
            finally:
    ...
    async def run_transcription(self, item_id: str, audio_payload: str) -> str:
        ...
        try:
            ...
            await self.emit_output(
                TranscriptionFailure(
                    item_id, getattr(exc, "code", "transcription_failed"), str(exc)
                )
            )
            raise
        finally:
    ...
    def _check_history_budget(
        ...
        # Note (Junnan Li): Shared sessions use the configured character budget for streamed text
        # and cumulative retained context. Legacy sessions leave it unset.
    ...
    def build_response_request(self, audio_payload: str) -> GenerateRequest:
        ...
        turn.
        """
        # Note (Junnan Li): Queued turns reach this boundary after earlier turns changed history;
        # input-admission checks cannot protect the eventual model request.
...
class RealtimeSession(TurnBasedSession):
    """Compatibility WebSocket boundary for the existing conversation API."""
