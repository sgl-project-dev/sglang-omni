...
| **Apple Silicon** | Experimental | **Qwen3-ASR and Whisper** run through native MLX or Torch MPS on macOS arm64, from the official checkpoints. Install with [`install.sh`](./install.sh), then follow the [Qwen3-ASR guide](./docs/cookbook/qwen3_asr.md#apple-silicon-mlx) or the [Whisper guide](./docs/cookbook/whisper_asr.md#apple-silicon-mlx). |
